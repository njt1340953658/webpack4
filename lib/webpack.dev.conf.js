...
const env_local = require('../config/dev.env')
...
const devWebpackConfig = merge(baseWebpackConfig, {
  ...
  plugins: [
    ...
    new AddAssetHtmlWebpackPlugin({
      filepath: utils.resolve('static/js/vendor.dll.js'), // 这个路径是你的dll文件路径 
      includeSourcemap: false // 这里是因为我开启了sourcemap。 不这么写会报错。
    }),
    new AddAssetHtmlWebpackPlugin({
      filepath: utils.resolve('static/js/huatuo.js'), // 这个路径是你的dll文件路径 
      includeSourcemap: false // 这里是因为我开启了sourcemap。 不这么写会报错。
    })
  ]
})
...
// 用于处理端口重复问题
module.exports = utils.addPort(devWebpackConfig)